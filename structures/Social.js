...
class Social extends Command {
    ...
    try {
      ...
      if (Date.now() > getPayer.daily) {
        ...
        if (message.content.indexOf('-r') !== -1) {
          reminded = true;
          const time = '1 day',
            action = `claim daily in ${message.guild.name}`;
  ...
  async usrPay(message, payer, payee, amount) {
    ...
    try {
      ...
      if (getPayer.points < parseInt(amount)) {
        throw `Insufficient funds, you have ${getPayer.points} points.`;
      }

      const response = await message.client.awaitReply(message, `Are you sure you want to pay ${message.guild.member(payee).displayName} ${parseInt(amount)} points?\n\n(**y**es | **n**o)\n\nReply with \`cancel\` to cancel the message. The message will timeout after 60 seconds.`);

      if (['yes', 'y', 'confirm'].includes(response.toLowerCase())) {
        getPayer.points -= parseInt(amount);
        getPayee.points += parseInt(amount);
        await message.channel.send(`The payment of ${parseInt(amount)} points has been sent to ${message.guild.member(payee).displayName}.`);
  ...
  async cmdPay(message, user, cost, perms) {
    ...
    try {
      ...
      if (amount > score.points) throw `Insufficient funds, you need ${amount} points. Your current balance: ${score.points}`;
      score.points -= amount;
  ...
  async cmdRew(message, user, amount) {
    try {
      ...
      score.points += amount;
      this.client.points.set(`${message.guild.id}-${user}`, score);
      await message.channel.send(`Awarded ${parseInt(amount)} points to ${message.guild.member(user).displayName}.`);
      return;
    } catch (error) {
      throw error;
    }
  ...
  async cmdPun(message, user, amount) {
    try {
      ...
      score.points -= amount;
      this.client.points.set(`${message.guild.id}-${user}`, score);
      await message.channel.send(`Deducted ${parseInt(amount)} points from ${message.guild.member(user).displayName}.`);
      return;
    } catch (error) {
      throw error;
    }
