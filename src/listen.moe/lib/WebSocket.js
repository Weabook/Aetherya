...
const { opCodes, closeCodes } = require('./Constants.js');
...
class WebSocket extends EventEmitter {
  ...
  connect() {
    ...
    this.emit('debug', `[Web Socket] Attempting to connect with WebSocket library "${wsLib}"`);
  ...
  close(code) {
    this.emit('debug', `[Web Socket] Closing WebSocket with code ${code}`);
    this.stopHeartbeat();
  ...
  startHeartbeat(ms) {
    ...
    this.emit('debug', '[Web Socket] Starting heartbeat interval');
  ...
  stopHeartbeat() {
    if (!this.heartbeat) return;
    this.emit('debug', '[Web Socket] Stopping heartbeat interval');
  ...
  sendJSON(ob) {
    ...
    this.emit('debug', `[Web Socket] -> ${stringify}`);
  ...
  onOpen() {
    ...
    this.emit('debug', '[Web Socket] WebSocket open');
  ...
  onMessage(d) {
    ...
    this.emit('debug', `[Web Socket] <- ${JSON.stringify(pkt)}`);
    ...
    switch (pkt.op) {
      case OPCodes.IDENTIFY:
        ...
        this.emit('debug', '[Web Socket] WebSocket ready');
        this.emit('ready', pkt);
        break;
      case OPCodes.HBACK:
        this.emit('hback');
  ...
  onError(err) {
    ...
    this.emit('debug', `[Web Socket] Error: ${err ? err.message ? err.message : JSON.stringify(err) : err}`);
    this.emit('error', err);
  ...
  onClose(code) {
    ...
    this.emit('debug', `[Web Socket] Closed WebSocket with code ${code}`);
  ...
  doReconnect() {
    if (!this.reconnect && this.autoReconnect) {
      this.emit('debug', '[Web Socket] Waiting 5000ms before reconnecting');
